...
# ------------------- BERT Tokenizer -------------------
# user: self.smallbert = BertModel.from_pretrained('prajjwal1/bert-small')
# => tokenizer도 동일 체크포인트 기반으로 맞춤
...
def text_prompt():
    ...
    Expected JSON (either):
      ...
      B) {"n1": 2, "n2": 1, "n3": 0}  (optional legacy/alt)
    ...
    # 1) legacy/alt numeric payload 지원: {"n1","n2","n3"}
    ...
    # 3) text 안이 숫자 3개 패턴이면 numeric으로 처리
    ...
    return jsonify({'status': 'success', 'active_text': active_text_prompt})
# (선택) 기존 /text_prompt 유지하고 싶으면 남겨도 되는데,
# 완전 교체 원하면 지워도 됨. 여기서는 혼동 방지로 제거 권장.
# @app.route('/text_prompt', methods=['POST'])
# def text_prompt():
#     ...
...
# NEW: send tokenized prompt (JSON)
...
# NEW: dual (mask + tokens) as multipart
def send_mask_and_tokens(socket, mask: np.ndarray, text: str, frame_idx: int, class_id: int):
    input_ids, attention_mask = tokenize_prompt(text)
...
def zmq_loop(socket, use_text, use_masks, use_dual):
    ...
    while True:
        ...
        text = active_text_prompt or ""   # sticky text

        if use_dual:
            send_mask_and_tokens(socket, mask, text, frame_idx, current_class)
            ...
            # 아무 옵션도 없으면 ack 정도라도 보내야 REP가 막히지 않음
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_text', action='store_true', help='Enable tokenized prompt forwarding')
    parser.add_argument('--use_masks', action='store_true', help='Enable mask forwarding')
    parser.add_argument('--use_dual', action='store_true', help='Enable dual (mask + tokenized prompt) forwarding')
